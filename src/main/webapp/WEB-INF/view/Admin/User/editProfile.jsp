...
<rapid:override name="content">
    ...
    <form class="layui-form" action="/admin/profile/save" id="userForm" method="post">
        <input type="hidden" name="userId" id="userId" value="${user.userId}" />
        <div class="layui-form-item">
            <label class="layui-form-label">头像</label>
            <div class="layui-input-inline">
                <img src="${user.userAvatar}" id="avatarImg" width="100" height="100">
                <p id="uploadImgTip"></p>
                <button type="button" class="layui-btn" id="uploadImgButton">上传图片</button>
...
<script>
    layui.use('upload', function() {
        ...
        var uploadInst = upload.render({
            ...
            error: function () {
                var uploadImgTip = $('#uploadImgTip');
                uploadImgTip.html('' +
                    '<span style="color: #FF5722;">上传失败</span>' +
                    ' <a class="layui-btn layui-btn-mini demo-reload">重试</a>');
                uploadImgTip.find('.demo-reload').on('click', function () {
                    uploadInst.upload();
                });
            }
        })
